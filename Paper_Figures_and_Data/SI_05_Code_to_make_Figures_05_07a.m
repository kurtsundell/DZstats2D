...
% make1D and 2D kdes and 2D cdfs for samples, save as 3D matrix
for k = 1:N
	data2 = data1(:,k*2-1:k*2);
	data2 = data2(any(data2 ~= 0,2),:);
	kde1(:,k) = kde(data2(:,1), ones(length(data2(:,1)),1)*bandwidth_x, xmin, xmax, xstep);
	clear data2
end

for j = 1:N
	for k = 1:N
		R1Dtest2(j,k) = r2b(kde1(:,j),kde1(:,k));
		R1Dtest(j,k) = r2(kde1(:,j),kde1(:,k));
		R1D(j,k) = ((sum((kde1(:,j) - mean(kde1(:,j))).*(kde1(:,k) - mean(kde1(:,k)))))/(sqrt((sum((kde1(:,j) - mean(kde1(:,j))).*(kde1(:,j)...
			- mean(kde1(:,j)))))*(sum((kde1(:,k) - mean(kde1(:,k))).*(kde1(:,k) - mean(kde1(:,k))))))))^2;
		L1D(j,k) = 1 - sum(abs(kde1(:,j) - kde1(:,k))./2);
		if j == k
			S1D(j,k) = 1;
		else
			S1D(j,k) = sum(((kde1(:,j).*kde1(:,k)).^0.5));	
		end
		[~,~,D1D(j,k)] = kstest2(nonzeros(data1(:,j*2-1)),nonzeros(data1(:,k*2-1)));
		[~,V1D(j,k)] = kuipertest2c(nonzeros(data1(:,j*2-1)),nonzeros(data1(:,k*2-1)));		
	end
end

%ANDES composites
%colours = [0,0,1;0,0.500000000000000,1;0,1,1;0.500000000000000,1,0.500000000000000;1,1,0;1,0.500000000000000,0;1,0,0;0.500000000000000,0,0;0,0,0];
%shapes = {'o';'o';'o';'o';'o';'o';'o';'o';'o'}

colours = [0,0,1;0,0,1;0,0.500000000000000,1;0,0.500000000000000,1;0,1,1;0,1,1;0.500000000000000,1,0.500000000000000;0.500000000000000,1,0.500000000000000;1,1,0;1,1,0;1,0.500000000000000,0;1,0.500000000000000,0;1,0,0;1,0,0;0.500000000000000,0,0;0.500000000000000,0,0;0,0,0];
shapes = {'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'s'};

%colours = [0,0,1;0,0,1;0,0.500000000000000,1;0,0.500000000000000,1;0,1,1;0,1,1;0.500000000000000,1,0.500000000000000;0.500000000000000,1,0.500000000000000;1,1,0;1,1,0;1,0.500000000000000,0;1,0.500000000000000,0;1,0,0;1,0,0;0.500000000000000,0,0;0.500000000000000,0,0];
%shapes = {'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s';'d';'s'};

[XY,stress,disparities] = mdscale_new(1-R1D,2,'Criterion','stress'); % 2D MDS plot and metric squared stress only
X=XY(:,1);
Y=XY(:,2);
dx=0.01;
figure
hold on
for i=1:N
	scatter(X(i),Y(i),250, char(shapes(i,1)), 'MarkerFaceColor',colours(i, :),'MarkerEdgeColor','black');
end
[rubbish,i] = sort(R1D,1,'descend');
YX=XY(i(2,:),:);
YZ=XY(i(3,:),:);
text(X+dx,Y+dx,Name,'FontSize',20);
title('Cross correlation 1D')

[XY,stress,disparities] = mdscale_new(1-L1D,2,'Criterion','stress'); % 2D MDS plot and metric squared stress only
X=XY(:,1);
Y=XY(:,2);
dx=0.01;
figure
...
[rubbish,i] = sort(L1D,1,'descend');
YX=XY(i(2,:),:);
YZ=XY(i(3,:),:);
text(X+dx,Y+dx,Name,'FontSize',20);
title('Likeness 1D')

[XY,stress,disparities] = mdscale_new(1-S1D,2,'Criterion','stress'); % 2D MDS plot and metric squared stress only
X=XY(:,1);
Y=XY(:,2);
dx=0.01;
figure
...
[rubbish,i] = sort(S1D,1,'descend');
YX=XY(i(2,:),:);
YZ=XY(i(3,:),:);
text(X+dx,Y+dx,Name,'FontSize',20);
title('Similarity 1D')

[XY,stress,disparities] = mdscale_new(D1D,2,'Criterion','stress'); % 2D MDS plot and metric squared stress only
X=XY(:,1);
Y=XY(:,2);
dx=0.01;
figure
...
[rubbish,i] = sort(D1D,1,'descend');
YX=XY(i(2,:),:);
YZ=XY(i(3,:),:);
text(X+dx,Y+dx,Name,'FontSize',20);
title('KS Test D 1D')

[XY,stress,disparities] = mdscale_new(V1D,2,'Criterion','stress'); % 2D MDS plot and metric squared stress only
X=XY(:,1);
Y=XY(:,2);
dx=0.01;
figure
...
[rubbish,i] = sort(V1D,1,'descend');
YX=XY(i(2,:),:);
YZ=XY(i(3,:),:);
text(X+dx,Y+dx,Name,'FontSize',20);
title('Kupier Test V 1D')
